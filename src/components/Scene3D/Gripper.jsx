...
function Gripper({ angle = 0, color = '#e74c3c', ...props }) {
  ...
  const openAmount = (angle / 90) * 0.15;

  useFrame(() => {
    if (leftFinger.current && rightFinger.current) {
      leftFinger.current.position.x = -0.1 - openAmount;
      rightFinger.current.position.x = 0.1 + openAmount;
    }
  });
  ...
  return (
    <group {...props}>
      ...
      <group ref={leftFinger}>
        <mesh position={[-0.1, -0.1, 0]} rotation={[0, 0, 0.1]}>
